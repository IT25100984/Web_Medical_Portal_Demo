...
public class HealthRecordDAO {
    ...
    public HealthRecord getHealthRecordById(int healthRecordID) {
        String sql = "SELECT health_record_id, patient_id, doctor_id, appointment_id, record_type, diagnosis, symptoms, treatment_plan, clinical_notes, allergies, medications, follow_up_instructions, created_at, updated_at FROM health_records WHERE health_record_id = ?";
        List<HealthRecord> records = jdbcTemplate.query(sql, healthRecordRowMapper(), healthRecordID);
        return records.isEmpty() ? null : records.get(0);
    }
    public List<HealthRecord> getHealthRecordsByPatientId(int patientID) {
        String sql = "SELECT health_record_id, patient_id, doctor_id, appointment_id, record_type, diagnosis, symptoms, treatment_plan, clinical_notes, allergies, medications, follow_up_instructions, created_at, updated_at FROM health_records WHERE patient_id = ? ORDER BY created_at DESC";
    ...
    public List<HealthRecord> getHealthRecordsByPatientUserId(int userID) {
        String sql = "SELECT hr.health_record_id, hr.patient_id, hr.doctor_id, hr.appointment_id, hr.record_type, hr.diagnosis, hr.symptoms, hr.treatment_plan, hr.clinical_notes, hr.allergies, hr.medications, hr.follow_up_instructions, hr.created_at, hr.updated_at FROM health_records hr JOIN patients p ON hr.patient_id = p.patient_id WHERE p.user_id = ? ORDER BY hr.created_at DESC";
        return jdbcTemplate.query(sql, healthRecordRowMapper(), userID);
    }
    public boolean updateHealthRecord(HealthRecord healthRecord) {
        String sql = "UPDATE health_records SET record_type = ?, diagnosis = ?, symptoms = ?, treatment_plan = ?, clinical_notes = ?, allergies = ?, medications = ?, follow_up_instructions = ? WHERE health_record_id = ?";
        int rowsAffected = jdbcTemplate.update(sql, normalizeRecordType(healthRecord.getRecordType()), healthRecord.getDiagnosis(), healthRecord.getSymptoms(), healthRecord.getTreatmentPlan(), healthRecord.getClinicalNotes(), healthRecord.getAllergies(), healthRecord.getMedications(), healthRecord.getFollowUpInstructions(), healthRecord.getHealthRecordID());
