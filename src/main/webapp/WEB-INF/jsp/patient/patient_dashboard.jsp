...
<html>
<head>
    <title>Patient Dashboard | WMP</title>
...
<div class="container mt-4 mb-5">
    ...
    <div class="p-5 mb-4 text-black rounded-3 shadow"
         style="background: linear-gradient(135deg, #ffffff -15%, #7dd3fc 20%, #38b6ff 60%, #0284c7 100%);">
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex flex-column gap-2" style="width: 200px;">
                    ...
                    <form action="${pageContext.request.contextPath}/profile/delete" method="POST" onsubmit="return confirm('WARNING: Are you absolutely certain you want to permanently delete your profile? This cannot be undone.');" style="margin-top: 10px; width: 100%;">
                                                    ...
                                                    <div class="row g-3">
                                                        ...
                                                        <div class="col-6 text-end">
                                                            ...
                                                            <p class="fw-bold text-success mb-0">
                                                                LKR <fmt:formatNumber value="${appt.totalFee}" type="number" minFractionDigits="2" maxFractionDigits="2"/>
                                                            </p>
                                                        </div>
    ...
    <%-- My Prescription Orders Card --%>
    <div class="card shadow border-0 mb-4">
        <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="bi bi-capsule me-2"></i>My Prescription Orders</h5>
            <a href="${pageContext.request.contextPath}/orderPrescription" class="btn btn-sm btn-light text-success fw-bold">
                <i class="bi bi-plus-lg me-1"></i>New Order
            </a>
        </div>
        <div class="card-body p-0 prescription-container">
            <table class="table table-hover mb-0 align-middle">
                <thead class="table-dark">
                <tr>
                    <th>Order #</th>
                ...
                <tbody>
                <c:choose>
                    <c:when test="${not empty myPrescriptions}">
                        <c:forEach var="pres" items="${myPrescriptions}">
                            <tr>
                                <td><strong>#${pres.prescriptionID}</strong></td>
                                <td><i class="bi bi-prescription2 me-1 text-primary"></i> ${pres.medicineName}</td>
                                <td>${pres.quantity}</td>
                                <td>LKR <fmt:formatNumber value="${pres.medicinePrice}" type="number" minFractionDigits="2" maxFractionDigits="2"/></td>
                                <td>
                                    <c:choose>
                                        <c:when test="${pres.status == 'PENDING'}">
                                            <span class="badge bg-warning text-dark"><i class="bi bi-clock me-1"></i>PENDING</span>
                                        </c:when>
                                        <c:when test="${pres.status == 'FILLED' || pres.status == 'READY' || pres.status == 'COMPLETED'}">
                                            <span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>${pres.status}</span>
                                        </c:when>
                                        <c:when test="${pres.status == 'CANCELLED'}">
                                            <span class="badge bg-danger"><i class="bi bi-x-circle me-1"></i>CANCELLED</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="badge bg-secondary">${pres.status}</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
                    ...
                    <c:otherwise>
                        <tr><td colspan="5" class="text-center py-4 text-muted">No prescription orders found.</td></tr>
                    </c:otherwise>
                </c:choose>
                </tbody>
            </table>
